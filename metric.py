...
class MonthlyAverageLoss(nn.Module):
    ...
    def month_data(self, input_data):
        # monthly_scatter = torch.zeros((len(self.months),1), dtype=torch.float32)
        month_indicies = torch.from_numpy(self.months).to(device).to(torch.int64).view(-1, 1)
        mean = scatter_mean(input_data.squeeze(), month_indicies.squeeze().to(input_data.device))
        return mean 
    ...
    def forward(self, output, target):
        ...
        # Compute the difference between ECDFs
...
class MonthlyMaxLoss(nn.Module):
    ...
    def month_data(self, input_data):
        # monthly_scatter = torch.zeros((len(self.months),1), dtype=torch.float32)
        month_indicies = torch.from_numpy(self.months).to(device).to(torch.int64).view(-1, 1)
        mean, _ = scatter_max(input_data.squeeze(), month_indicies.squeeze().to(input_data.device))
        return mean
